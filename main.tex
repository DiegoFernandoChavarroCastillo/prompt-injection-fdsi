...
\author{Laura Alejandra Venegas Piraban, David Palacios, Diego Fernando Chavarro}
...
\begin{abstract}
% PENDIENTE: escribir de último, cuando existan los resultados (150-250 palabras).
% Debe responder: (1) qué problema se aborda, (2) qué se hizo, (3) qué se
% encontró (ASR antes/después y FPR), (4) qué se concluye.
\end{abstract}
...
En primer lugar, prevalece una evaluación aislada de las propuestas. Por un lado, las investigaciones centradas en vulnerabilidades \cite{ref1}, \cite{ref10} suelen catalogar técnicas de ataque sin medir sistemáticamente su impacto contra sistemas fortificados. Por otro lado, los estudios enfocados en defensas \cite{ref6}, \cite{ref7}, \cite{ref8}, \cite{ref9} acostumbran validar sus soluciones utilizando conjuntos de ataque diseñados a la medida, omitiendo la comparación directa contra una línea base vulnerable bajo condiciones idénticas.
...
\begin{itemize}
    ...
    (\texttt{\textless\textless\textless USER\_DATA\_a91f\textgreater\textgreater\textgreater{} ... \textless\textless\textless/USER\_DATA\_a91f\textgreater\textgreater\textgreater}) y se
    ...
    \item[] \textit{Nota de implementación:} el recordatorio de L2 se
    reinyecta dentro del mismo mensaje \texttt{user}, inmediatamente después
    del delimitador de cierre del bloque de datos. De este modo la última
    instrucción presente en el contexto procede del operador y no del
    cliente. Si la entrada del usuario contiene una marca con la forma de un
    delimitador ---en cualquier variante de mayúsculas, espaciado o
    identificador--- se sustituye por un marcador neutro antes de
    encapsularla, para que no pueda cerrar el bloque de datos y escribir
    fuera de él.
...
\node[seg, fill=black!12, below=0pt of l4] (l1)
  {\textbf{L1} \texttt{\textless\textless\textless USER\_DATA\textgreater\textgreater\textgreater}\\ entrada del usuario delimitada
   \\\texttt{\textless\textless/USER\_DATA\textgreater\textgreater\textgreater}};
...
modificó con posterioridad}, extremo verificable en el historial del
repositorio. La capa L5 no quedó expuesta, ya que en una pasada en seco no
...
contrario, pues sorteó L3 y aun así fue rechazado en la condición B--- sino que
este diseño no puede pronunciarse sobre ellas con la evidencia disponible. Dos hipótesis quedan abiertas y son indistinguibles
...
literatura.} Los trabajos que documentan estas técnicas \cite{ref1},
\cite{ref10} describen tasas de éxito sustancialmente mayores que el 15\,\%
observado aquí contra la condición A. La explicación más plausible es la
...
Se utilizó Claude Code (Anthropic) como asistente de programación para
implementar el código del experimento. El diseño experimental, la revisión de las
decisiones metodológicas y la interpretación de los resultados son
